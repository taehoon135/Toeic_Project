...
class Wordbook:
    ...
    def edit_word(self):
        ...
        """
        print("\n=== 단어 수정 ===")
        word_id = input("수정할 단어의 번호를 입력하세요: ")
        
        try:
            # 단어 존재 여부 확인
            self.cursor.execute("SELECT * FROM Word WHERE word_id = ?", (word_id,))
            if not self.cursor.fetchone():
                print("해당 번호의 단어가 없습니다.")
                return
            
            english = input("영어 단어를 입력하세요 (변경 없으면 Enter): ")
            meaning = input("의미를 입력하세요 (변경 없으면 Enter): ")
            pos = input("품사를 입력하세요 (변경 없으면 Enter): ")
            example = input("예문을 입력하세요 (변경 없으면 Enter): ")
            ...
            if updates:
                ...
                query = f"UPDATE Word SET {', '.join(updates)} WHERE word_id = ?"
